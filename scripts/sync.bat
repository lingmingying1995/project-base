@echo off
chcp 65001 >nul
REM Git auto sync script (template, Windows)
REM Triggered by Task Scheduler daily (see scripts/README.md for setup)
REM Flow: stash -> pull --rebase -> pop -> commit -> push, retry 3 times on fail
REM Task log: written by write-task-log.ps1 (Chinese dir name auto-handled)
REM
REM Usage: set PROJECT_NAME below, then create scheduled task pointing to this file
